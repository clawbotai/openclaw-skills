...
Infrastructure code differs from application code: bugs here cost real money and cause outages. This skill uses a strict lifecycle to ensure safety and continuous improvement.

### Part 1: Evolutionary Loop

1.  **Phase 1 — Research**: Before writing IaC or pipelines, define the architecture in `SPECIFICATION.md`. 
    *   *DevOps Specifics*: Identify API limits, state locking strategies, and security implications of new resources.
2.  **Phase 2 — Build**: Iterative implementation with strict backpressure gates.
    *   *Gates*: `terraform validate` → `tflint` → `docker build` → `checkov` (security scan).
    *   *Progress*: Log every plan/apply cycle in `PROGRESS.md`.
3.  **Phase 3 — Reflect**: Post-deployment verification. If a pipeline breaks or infrastructure drifts, generate a repair ticket and update the specification.

### Part 2: Runtime Monitor

The runtime monitor intercepts DevOps operations to prevent destructive loops and alert on systemic failures.

**Error Classification**
*   **Transient**: GitHub API 503, AWS rate limits, temporary DNS failures. *Action: Retry with exponential backoff.*
*   **Deterministic**: `Terraform syntax error`, `Docker build failed`, `Invalid credentials`, `Policy violation`. *Action: Generate Repair Ticket & Trip Circuit Breaker.*

**Circuit Breaker**
Trips after 5 consecutive failures to prevent API bans, account lockouts, or billing spikes from run-away automation.
...
```
# Monitor
python3 scripts/monitor.py status       # Check health & alerts
python3 scripts/monitor.py tickets      # View repair tickets
python3 scripts/monitor.py reset <skill> # Reset circuit breaker
...
This skill participates in the multi-agent workflow system. When deploying, building infrastructure, or running CI/CD as part of an orchestrated pipeline, emit structured state events.
...
# Load the work item assigned by the orchestrator
wi = load_item("wf-deploy-staging-stage-2")
wi.start(assignee_skill="devops", author="devops")

# Record artifacts as you produce them
wi.add_artifact({"type": "file", "label": "Dockerfile", "path": "Dockerfile"})
wi.add_artifact({"type": "file", "label": "CI pipeline", "path": ".github/workflows/deploy.yml"})
wi.add_artifact({"type": "link", "label": "Deploy URL", "path": "https://staging.example.com"})

# Record deployment metrics
wi.record_metric({"name": "build_time", "value": 42, "unit": "seconds"})
wi.record_metric({"name": "image_size", "value": 187, "unit": "MB"})

# Record health check results as tests
wi.record_test({"name": "health_check", "status": "pass", "evidence": "HTTP 200 on /health"})
wi.record_test({"name": "smoke_test", "status": "pass", "evidence": "All critical endpoints responding"})

# Findings from deployment
wi.add_finding("ARM64 builds need --platform linux/arm64 explicitly on CI runners")

# On success
wi.complete(author="devops")
```
...
| Moment | Event | Why |
|--------|-------|-----|
| Starting deployment/infra work | `wi.start()` | Orchestrator tracks; timing starts |
| Dockerfile, compose, pipeline created | `wi.add_artifact()` | Sanity-check verifies; docs-engine documents |
| Build/deploy completes | `wi.record_metric()` | Skill-lifecycle monitors build-time drift |
| Health/smoke checks | `wi.record_test()` | Gating: blocks promotion if deployment unhealthy |
| Infrastructure gotcha discovered | `wi.add_finding()` | Reflect encodes for future deployments |
| Deployment verified | `wi.complete()` | Triggers post-hook chain |
| Deployment failed | `wi.fail(reason=...)` | Orchestrator halts; skill-lifecycle opens repair ticket |

### Handoff Patterns

DevOps typically receives from python-backend/web-builder and hands off to observability or sanity-check:

```python
# Receive: orchestrator routes to devops after code is ready
wi = load_item(slug)  # Already has artifacts from python-backend
wi.start(assignee_skill="devops", author="devops")

# After successful deploy, hand off to monitoring
wi.handoff(to_skill="observability", author="devops")
```

### Contract Reference

This skill's contract is at `config/skill_contracts/devops.json`. It declares:
- **Capabilities:** ci_cd, infrastructure, containers, kubernetes, gitops, monitoring, deploy, security_scan
- **Inputs:** source_files (required), deploy_target, infra_config
- **Outputs:** pipeline_config, container_config, deploy_logs, metrics
- **Upstream:** python-backend, web-builder
- **Downstream:** observability, sanity-check, docs-engine